...
# -----------------------------
# 4. BOOKINGS + QUEUE
# (WITH EXPLICIT CULTIVATION REFERENCES)
# -----------------------------

# Track remaining production per cultivation
remaining_production = {
    i: {
        "passbook": rec[1],
        "crop": rec[4],
        "produced": float(rec[5]),
        "remaining": float(rec[5])
    }
    for i, rec in enumerate(cultivation_records)
}
...
# Shuffle cultivation records to distribute bookings
cultivation_indices = list(range(len(cultivation_records)))
random.shuffle(cultivation_indices)

bookings_per_cultivation = 500 // len(cultivation_records) + 1

for cultivation_idx in cultivation_indices:
    
    cultivation_record = cultivation_records[cultivation_idx]
    cultivation_id_str = cultivation_record[0]
    passbook = cultivation_record[1]
    crop = cultivation_record[4]
    remaining = remaining_production[cultivation_idx]["remaining"]
    
    # Random number of bookings for this cultivation (0-3)
    num_bookings = random.randint(0, min(3, bookings_per_cultivation))
    ...
    for _ in range(num_bookings):
        ...
        # Ensure booking doesn't exceed remaining
        max_sellable = remaining
        quantity_to_sell = round(
            random.uniform(0.5, max_sellable),
            2
        )
        
        if quantity_to_sell > remaining:
            quantity_to_sell = remaining
        ...
        remaining_production[cultivation_idx]["remaining"] -= quantity_to_sell
        booking_id += 1
...
# -----------------------------
# 5. PROCUREMENT + PAYMENT (FROM VALID BOOKINGS ONLY)
# -----------------------------
...
for booking in bookings:
    ...
    # booking[8] is queue_status (was index 6 before adding cultivation_id/crop)
    if booking[8] != "COMPLETED":
        continue

    # booking[7] is quantity_to_sell_quintals (was index 5 before)
    quantity_submitted = float(booking[7])
